...
public class Join {
    ...
    public static class customerMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            context.write(new Text(fields[0]), new Text("customers : " + fields[1])); //id and customer_name
        }
    }
    ...
    public static class orderMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            String[] fields = line.split("\\|");
            int n =  fields.length;
            context.write(new Text(fields[1]), new Text("orders : " +fields[n-1]));//customer id and orders comment
        }
    }
        ...
        public void reduce(Text key, Iterable<Text> values, Context context)
            ...
            for (Text val : values) {
                ...
                if (s.startsWith("customers : ")) {
                    customers.add(s.substring("customers : ".length()));
                } else if (s.startsWith("orders : ")) {
                    orders.add(s.substring("orders : ".length()));
                }
            }
            ...
            for (String customerName : customers) {
                for (String orderComment : orders) {
                    context.write(new Text(customerName), new Text(orderComment));
                }
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        MultipleInputs.addInputPath(job,
                new Path("input-join/orders.tbl"),
                TextInputFormat.class,
                orderMapper.class);
